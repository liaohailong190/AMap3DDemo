...
//        MapsInitializer.sdcardDir =OffLineMapUtils.getSdCacheDir(this);
    ...
    private void addOverlayToMap() {
        amap.moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(39.936713,
                116.386475), 3.0f));// 设置当前地图显示为北京市恭王府
        ...
        amap.setOnCameraChangeListener(new AMap.OnCameraChangeListener() {
            ...
            public void onCameraChangeFinish(CameraPosition cameraPosition) {
                ...
                float width = 1920 * scalePerPixel;
                float height = 1080 * scalePerPixel;
                LatLng target = amap.getCameraPosition().target;
                BitmapDescriptor bd = BitmapDescriptorFactory.fromResource(R.drawable.default_background);
                GroundOverlayOptions gop = new GroundOverlayOptions()
                        .anchor(0.5f, 0.5f)
                        .image(bd)
                        .transparency(0.45f)
                        .position(target, width, height);
                amap.addGroundOverlay(gop);
            }
        });
